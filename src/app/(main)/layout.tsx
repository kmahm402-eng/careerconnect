    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-5xl mx-auto flex">
        ...
        <aside className="hidden lg:block w-[280px] shrink-0 sticky top-0 h-screen pt-6 px-4 overflow-y-auto">
          ...
          <div className="bg-card rounded-2xl border p-4"><h3 className="font-semibold text-sm mb-3">積極採用中</h3><div className="space-y-3">{[{ n: "Mobile Next", c: "docomo" }, { n: "Comm Partner", c: "au" }, { n: "Smart Life", c: "SoftBank" }].map((co, i) => (<div key={i} className="flex items-center justify-between"><div><p className="text-sm font-medium">{co.n}</p><p className="text-xs text-muted-foreground">{co.c}</p></div><span className="text-xs font-medium text-green-600 bg-green-50 px-2 py-0.5 rounded-full">採用中</span></div>))}</div></div>
        </aside>
      </div>
      {/* Mobile Bottom Nav */}
