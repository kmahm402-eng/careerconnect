...
export default function ChatPage() {
  ...
  async function fetchData() {
    ...
    if (!user) return;
    const { data: seeker } = await (supabase.from("seekers") as any).select("id").eq("auth_user_id", user.id).single();
    if (seeker) setSeekerId(seeker.id);
    const { data: comp } = await (supabase.from("companies") as any).select("id, company_name, is_verified, carrier").eq("id", companyId).single();
    if (comp) setCompany(comp);
    if (seeker) {
      const { data: msgs } = await (supabase.from("messages") as any).select("*").eq("company_id", companyId).eq("seeker_id", seeker.id).order("created_at", { ascending: true });
  ...
  async function handleQuickAction(action: "interested_now" | "maybe" | "pass") {
    ...
    const supabase = createClient();
    if (action === "pass") {
      await (supabase.from("messages") as any).update({ status: "archived", archived_at: new Date().toISOString() }).eq("company_id", companyId).eq("seeker_id", seekerId);
    ...
    const { data: newMsg } = await (supabase.from("messages") as any).insert({ company_id: companyId, seeker_id: seekerId, sender_type: "seeker", content }).select().single();
  ...
  async function handleSendMessage(text: string) {
    if (!seekerId) return;
    const supabase = createClient();
    const { data: newMsg } = await (supabase.from("messages") as any).insert({ company_id: companyId, seeker_id: seekerId, sender_type: "seeker", content: text }).select().single();
    if (newMsg) setMessages((prev) => [...prev, newMsg]);
  }
